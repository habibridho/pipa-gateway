...
PipaGateway.prototype.__init__routes = function(methodPath, config) {
  ...
  switch (type) {
    case "amarthaBridge":
      middlewares.push(
        self.__route__handler.amarthaBridge(self.domain, httpMethod, config)
      );
      constructMiddlewares();
      break;
    case "proxy":
      constructMiddlewares();
      middlewares.push(
...
PipaGateway.prototype.__route__handler = {
  /*
  *   amarthaBridge
  *   Direct call to a service then proceed to other middlewares.
  */

  amarthaBridge: function(domain, httpMethod, config) {
    return function(req, res, next) {
      ...
      request(opts, function(err, response, body) {
        ...
        req.oldServiceStatus = response.statusCode
        req.oldServiceBody = body
        next();
      });
    };
  },
